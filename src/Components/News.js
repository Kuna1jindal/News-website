    ...
    loadpage=async(page)=> {
        ...
        if(parserdata["status"]==="ok"){
            if(page===1){
                ...
                this.setState({articles:this.state.articles.concat(parserdata["articles"]),})
            }
            this.setState({
            loading:false,
            page:page,
        ...
        this.setState({articles:[],totalResults:0,loading:true}); 
    }
...
fetchMoreData =async() => {
  ...
  render() {
    let defaultimage="https://imgs.search.brave.com/mgID5Uw3yQNrUvHRTTM6mnHIJ_ddQ72hjsus1DqLJK4/rs:fit:500:0:0/g:ce/aHR0cHM6Ly9zdDQu/ZGVwb3NpdHBob3Rv/cy5jb20vMTMzMjQy/NTYvMjMzODkvaS80/NTAvZGVwb3NpdHBo/b3Rvc18yMzM4OTg0/MzAtc3RvY2stcGhv/dG8tY3JvcHBlZC1p/bWFnZS1qb3VybmFs/aXN0LWhvbGRpbmct/bmV3c3BhcGVyLmpw/Zw";
      return (
        <>
         
              <h1 className='text-center'>News Monkey.Com</h1>
      ...
      <NewsItem  title={element.title?element["title"]:"Title is unavailable"} description={element.description?element["description"].slice(0,70):"Description is unavailable"} newsurl={element.url?element.url:""} imagesource={element["urlToImage"]?element["urlToImage"]:defaultimage} publishedAt={element.publishedAt} author={element.author} source={element.source.name}
    />
       </div>
   )
})}
